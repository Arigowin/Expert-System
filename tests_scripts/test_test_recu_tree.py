...
for i in range(100):
    process = Popen(["python3", "main.py", "tests/test_recu_tree.txt"], stdout=PIPE)
    (ret, err) = process.communicate()
    exit_code = process.wait()
    if (str(ret).find("[A:[1, 0, 3]]") == -1 or
        str(ret).find("[B:[2, 1, 2]]") == -1 or
        str(ret).find("[C:[1, 0, 3]]") == -1 or
        str(ret).find("[D:[0, 0, 0]]") == -1 or
        str(ret).find("[E:[0, 0, 0]]") == -1 or
        str(ret).find("[F:[0, 0, 0]]") == -1 or
        str(ret).find("[G:[0, 0, 0]]") == -1 or
        str(ret).find("[H:[2, 1, 2]]") == -1 or
        str(ret).find("[I:[0, 0, 0]]") == -1 or
        str(ret).find("[J:[0, 0, 0]]") == -1 or
        str(ret).find("[K:[1, 0, 3]]") == -1 or
