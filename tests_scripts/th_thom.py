...
print("\n", __file__)
for i in range(100):
    process = Popen(["python3", "main.py", "tests/thomas/testThom.txt"], stdout=PIPE)
    (ret, err) = process.communicate()
    exit_code = process.wait()
    if (str(ret).find("[A:[1, 2, 3]]") == -1 or
        str(ret).find("[B:[3, 2, 1]]") == -1 or
        str(ret).find("[C:[3, 2, 1]]") == -1 or
        str(ret).find("[D:[0, 2, 0]]") == -1 or
        str(ret).find("[E:[0, 2, 0]]") == -1 or
        str(ret).find("[F:[0, 0, 0]]") == -1 or
        str(ret).find("[G:[1, 0, 3]]") == -1 or
